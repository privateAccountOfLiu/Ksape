...
[![Version](https://img.shields.io/badge/version-0.1.2-89b4fa)](https://github.com/privateAccountOfLiu/Ksape/releases)

Ksape 是一个 KernelSU WebUI 模块，提供专业级安卓进程监控与管理功能。通过 KernelSU 的 root 权限直接读取 `/proc` 文件系统，实现深度进程信息查看和实时性能监控。

## 功能

- **进程列表** — 实时刷新，支持 PID/名称/CPU/内存排序、搜索过滤、进程状态筛选（All/Running/Sleeping/Disk/Stopped/Zombie）
...
- **Find Window Process** — 检测前台应用，自动匹配对应进程及详细信息
- **设置** — 刷新频率调节、内核线程过滤、系统进程过滤
- **响应式布局** — 适配窄屏/小窗模式，详情面板覆盖层+返回按钮
...
1. 在 [Releases](https://github.com/privateAccountOfLiu/Ksape/releases) 下载 `Ksape-*.zip`
...
└── webroot/                 # WebUI 根目录
    ...
    │   ├── settings.js      # 设置弹窗
    │   ├── sysinfo.js       # 系统信息弹窗
    │   ├── findwin.js       # Find Window Process 弹窗
    │   └── utils.js         # 格式化 + DOM 工具
    └── styles/
        ├── base.css         # Reset + CSS 变量
        ├── layout.css       # Grid 布局 + 响应式断点
        └── components.css   # 组件样式
```
...
- **发布/订阅状态** — 轻量级 `setState()`/`subscribe()` 模式，UI 组件订阅状态键自动重渲染

## 开发

```bash
git clone https://github.com/privateAccountOfLiu/Ksape.git
cd Ksape
npm install                 # 仅打包需要
npm run build               # 输出 dist/Ksape-v0.1.2.zip
...
| `head -1` | `head -n 1` |
| `tail -10` | `tail -n 10` |
| `renice 10 -p PID` | `renice -n 10 PID` |
| `sed -E` | `sed` (仅 BRE) |
...
- **Process List** — Real-time refresh with sorting by PID/name/CPU/memory, search/filter, and state filter (All/Running/Sleeping/Disk/Stopped/Zombie)
...
- **Find Window Process** — Detects foreground app and matches to process with detailed info
- **Settings** — Refresh interval, kernel thread filter, system process filter
- **Responsive Layout** — Adapts to narrow/portrait screens with detail panel overlay and back button

## Installation

1. Download `Ksape-*.zip` from [Releases](https://github.com/privateAccountOfLiu/Ksape/releases)
...
- **Pub/sub state** — Lightweight `setState()`/`subscribe()` pattern, UI components subscribe to state keys for auto re-render
...
npm run build               # Output: dist/Ksape-v0.1.2.zip
